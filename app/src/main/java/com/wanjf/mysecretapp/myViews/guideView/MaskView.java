package com.wanjf.mysecretapp.myViews.guideView;
...
class MaskView extends ViewGroup {
     ...
     * 目标区域样式，默认为矩形
     */
    private int mStyle = Component.ROUND_RECT;
    /**
     * 挖空画笔
     */
        ...
        if (!mOverlayTarget) {
            switch (mStyle) {
                ...
                case Component.CIRCLE:
                    ...
                    break;
                case Component.ROUND_RECT_AND_DOTTED_LINE: {// 外面带虚线的
                    @SuppressLint("DrawAllocation") DashPathEffect dpe = new DashPathEffect(new float[]{4, 4}, 0);
                    mEraser.setPathEffect(dpe);
                    mEraserCanvas.drawRoundRect(mTargetRect, mCorner, mCorner, mEraser);
                    break;
                }
                default:
